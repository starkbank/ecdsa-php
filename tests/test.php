...
$privateKeyPem = EcdsaPhp\Utils\File::read("tests/privateKey.pem");
$privateKey = EcdsaPhp\PrivateKey::fromPem($privateKeyPem);
$message = EcdsaPhp\Utils\File::read("tests/message.txt");
$signature = EcdsaPhp\Ecdsa::sign($message, $privateKey);
$publicKey = $privateKey->publicKey();
assertEqual(EcdsaPhp\Ecdsa::verify($message, $signature, $publicKey), true);
...
$publicKeyPem = EcdsaPhp\Utils\File::read("tests/publicKey.pem");
// openssl dgst -sha256 -sign privateKey.pem -out signature.binary message.txt
$signatureDer = EcdsaPhp\Utils\File::read("tests/signatureDer.txt");
$message = EcdsaPhp\Utils\File::read("tests/message.txt");
...
$publicKey2 = EcdsaPhp\PublicKey::fromPem($pem);
...
$publicKey2 = EcdsaPhp\PublicKey::fromDer($der);
assertEqual($publicKey1->toPem(), $publicKey2->toPem());
...
$publicKey2 = EcdsaPhp\PublicKey::fromString($str);
assertEqual($publicKey1->toPem, $publicKey2->toPem);
...
$signature2 = EcdsaPhp\Signature::fromDer($der);
...
$signature2 = EcdsaPhp\Signature::fromBase64($base64);
assertEqual($signature1->r, $signature2->r);
assertEqual($signature1->s, $signature2->s);
